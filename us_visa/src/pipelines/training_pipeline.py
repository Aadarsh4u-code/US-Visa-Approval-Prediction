...
from us_visa.src.components.data_transformation import DataTransformation
from us_visa.src.components.model_trainer import ModelTrainer
from us_visa.src.components.model_evaluation import ModelEvaluation
from us_visa.src.components.model_pusher import ModelPusher
...
class TrainPipeline:
    ...
    def run_pipeline(self, ) -> None:
        ...
        try:
            ...
            logging.info(f"{data_ingestion_artifact} {'='*50}")

            data_validation_artifact = self.start_data_validation(data_ingestion_artifact=data_ingestion_artifact)
            logging.info(f"{data_validation_artifact} {'='*50}")

            data_transformation_artifact = self.start_data_transformation(
                data_ingestion_artifact=data_ingestion_artifact, 
                data_validation_artifact=data_validation_artifact)
            logging.info(f"{data_transformation_artifact} {'='*50}")

            model_trainer_artifact = self.start_model_trainer(data_transformation_artifact=data_transformation_artifact)
            logging.info(f"{model_trainer_artifact} {'='*50}")
            
            model_evaluation_artifact = self.start_model_evaluation(data_ingestion_artifact=data_ingestion_artifact,
                                                                    model_trainer_artifact=model_trainer_artifact)
            logging.info(f"{model_evaluation_artifact} {'='*50}")
            ...
            logging.info(f"{model_pusher_artifact} {'='*50}")
